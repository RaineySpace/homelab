...
单一家庭内部使用。默认一个家庭、一个 owner 账号；owner 可维护 member/viewer 账号。界面中文，PC 与手机都能用。

## 用户故事

1. 作为家庭管理员，我可以用引导账号登录，管理普通账号及其家庭人物关联，并看到当前家庭会话。
2. 作为 member，我可以创建、查看、修改、归档家庭人物，并查看人物修订；作为 viewer，我只能查看家庭业务。
...
- Next.js 页面覆盖登录、人物、菜谱、配餐、任务、Agent、账号管理和密码设置
- Compose：reverse-proxy / web / api / backup

### 首期刻意做薄的部分

- 权限模型只有 owner/member/viewer：owner 管理账号，member 完整读写业务，viewer 只读；界面不提供邀请和 owner 转移。
...
1. `pnpm test` 覆盖：认证、账号管理与会话撤销、人物绑定/归档、人物 CRUD/修订/乐观锁/幂等、菜谱、配餐确认、任务、Agent 工具与 HTTP 写入同一人物。
